...
//			}
	...
	public final class OnPlayerPickUp implements Listener {
		private final JavaPlugin plugin;
		public OnPlayerPickUp(JavaPlugin plugin) { this.plugin = plugin; }
		...
		public void onItemPickup(EntityPickupItemEvent event) {
			...
			// SLIME BALL -> mat_vanilla_slime
			if (ItemManager.getId(picked) == ItemManager.getId(Material.SLIME_BALL) &&
					LoreCraftableMaterial.getCraftMaterial(picked) == null) {
				replacePickup(event, player, "mat_vanilla_slime", picked.getAmount());
				return;
			}

			// ENDER PEARL -> mat_ender_pearl
			if (ItemManager.getId(picked) == ItemManager.getId(Material.ENDER_PEARL) &&
					LoreCraftableMaterial.getCraftMaterial(picked) == null) {
				replacePickup(event, player, "mat_ender_pearl", picked.getAmount());
				return;
			}

			// TNT -> mat_vanilla_tnt
			if (ItemManager.getId(picked) == ItemManager.getId(Material.TNT) &&
					LoreCraftableMaterial.getCraftMaterial(picked) == null) {
				replacePickup(event, player, "mat_vanilla_tnt", picked.getAmount());
				return;
			}
		...
		private void replacePickup(EntityPickupItemEvent event, Player player, String craftId, int amount) {
			LoreCraftableMaterial mat = LoreCraftableMaterial.getCraftMaterialFromId(craftId);
			ItemStack newStack = LoreCraftableMaterial.spawn(mat);
			newStack.setAmount(amount);

			// zet in inventory; behandel leftover (vol inventory)
			Map<Integer, ItemStack> leftover = player.getInventory().addItem(newStack);
			if (!leftover.isEmpty()) {
				leftover.values().forEach(is ->
						player.getWorld().dropItemNaturally(player.getLocation(), is));
			}

			// stop vanilla pickup + verwijder entity
			event.setCancelled(true);
			event.getItem().remove();

			// resync **volgende tick** (voorkomt desync)
